...
function OrdersContent() {
    ...
    // Top-level tab
    const initialType = (searchParams.get('type') ?? 'tests') as OrderType;
    const [orderType, setOrderType] = useState<OrderType>(initialType === 'medicines' ? 'medicines' : 'tests');
            ...
            <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
                <div className="max-w-3xl mx-auto px-4 sm:px-6">
                    <div className="flex items-center gap-3 py-4">
                        <Link href="/account" className="p-2 rounded-xl text-gray-500 hover:bg-gray-100 transition-colors shrink-0">
                    ...
                    <div className="flex gap-1 pb-px">
                        {([
                            { key: 'tests',     label: 'Lab Tests',  icon: <FlaskConical className="h-4 w-4" /> },
                            { key: 'medicines', label: 'Medicines',   icon: <Pill className="h-4 w-4" /> },
                        ] as { key: OrderType; label: string; icon: React.ReactNode }[]).map(t => (
                            <button
                                key={t.key}
