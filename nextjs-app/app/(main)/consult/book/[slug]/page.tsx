...
function SuccessOverlay({ data, onHome }: { data: SuccessData; onHome: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col items-center justify-center p-6 text-center">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 w-full max-w-sm mb-3">
        ...
        <p className="text-xs text-gray-500 mt-2">
          {data.consultationMode === 'video'
            ? 'Join the meeting link below at the scheduled time.'
            : 'Your doctor will call you at the scheduled time.'}
        </p>
      </div>

      {data.consultationMode === 'video' && data.meetLink && (
        <a
          href={data.meetLink}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 w-full max-w-sm bg-blue-600 text-white font-bold py-3 rounded-xl text-sm hover:bg-blue-700 transition-colors mb-3"
        >
      ...
      <div className="flex gap-3 w-full max-w-sm">
        ...
        <button
          onClick={onHome}
          className="flex-1 bg-gray-900 text-white font-bold py-3 rounded-xl text-sm hover:bg-gray-800 transition-colors"
        >
          Back to Home
        </button>
      </div>
...
export default function BookingPage({
  ...
  const submitAppointment = async (
    formData: FormValues,
    payment: { cfOrderId: string; cfPaymentId?: string } | null
  ) => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('authToken') : null;
    const body = {
      doctorSlug: slug,
  ...
  const onSubmit: SubmitHandler<FormValues> = async (formData) => {
    ...
    // Paid consultation — Cashfree flow
    setSubmitting(true);
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('authToken') : null;

      // Create Cashfree order
      const cfRes = await fetch('/api/payment/cashfree/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
        ...
        body: JSON.stringify({
          amount: finalAmount,
          orderRef: `consult_${Date.now()}`,
          customerName: formData.patientName,
          customerPhone: formData.mobile,
        }),
      });
      const cfData = await cfRes.json();
      if (!cfData.success) throw new Error(cfData.message || 'Payment initialization failed');

      const { cfOrderId, paymentSessionId } = cfData.data;
      ...
      if ((result as any).error) {
        const errMsg = (result as any).error?.message || 'Payment cancelled.';
        toastError(errMsg);
        return;
      }

      if ((result as any).paymentDetails || (result as any).redirect) {
        await submitAppointment(formData, { cfOrderId });
      }
    } catch (err: any) {
